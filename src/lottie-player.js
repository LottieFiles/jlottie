  ...
  for (var i = 1; i < frames; i++) {
    ...
    if (refKey != "ks") {
      ...
      for (var k = 0; k < p1[j].i.length; k++) {
        ...
        newNodes[newNodes.length - 1].s[
          newNodes[newNodes.length - 1].s.length - 1
        ].i.push([
    ...
    if (currentDate - animation[i]._lastTime >= animation[i]._frameTime) {
      ...
      if (animation[i]._currentFrame >= animation[i]._totalFrames) {
        ...
        if (!animation[i]._loop) {
          animation[i]._paused = true;
          lottie.goToAndStop(
            animation[i]._totalFrames - 1,
            "",
            animation[i]._elementId
          );
          continue;
        }
      }
      ...
      for (
        var j = 0;
        j < animation[i]._scene[animation[i]._currentFrame]._transform.length;
        j++
      ) {
        if (
...
function stageSequence(animationId, stageObj, inPoint, outPoint) {
  ...
  if (inPoint > -1) {
    ...
    for (
      ...
      if (
        animation[animationId]._scene[parseInt(frame)]._transform[i].stageObj ==
        stageObj
      ) {
        transforms =
...
function addGroupPositionTransform(
  ...
  if (isLayer) {
    if (animation[animationId].hasOwnProperty("_currentLayerGroup")) {
      if (animation[animationId]._currentLayerGroup._inPoint >= 0) {
        console.log("inpoint");
        transforms.inPoint = parseInt(
          animation[animationId]._currentLayerGroup._inPoint
        );
      }
      if (animation[animationId]._currentLayerGroup._outPoint > 0) {
        transforms.outPoint = parseInt(
          animation[animationId]._currentLayerGroup._outPoint
        );
      }
    } else {
      if (animation[animationId]._currentLayer._inPoint >= 0) {
        console.log("inpoint");
        transforms.inPoint = parseInt(
          animation[animationId]._currentLayer._inPoint
        );
      }
  ...
  if (isLayer) {
    ...
    if (objectId.td > 0) {
      transforms.refObj =
        animationId + "_" + depth + "_layerGroup" + objectId._layer;
    } else {
      transforms.refObj =
        animationId + "_" + depth + "_layerTranslate" + objectId._layer;
    }
  ...
  if (refKey == "r") {
    ...
    if (
      objectId.hasOwnProperty("_anchorX") &&
      objectId.hasOwnProperty("_anchorY")
    ) {
      ...
      transforms.rotate =
        ...
        transforms.rotateAngle +
        "," +
        document.getElementById(transforms.refObj).getBoundingClientRect()
          .width /
          2 +
        "," +
        document.getElementById(transforms.refObj).getBoundingClientRect()
  ...
  if (refKey == "s") {
    ...
    transforms.paddingX =
      (transforms.anchorX - tempBoundingW) * currentScaleX +
      tempBoundingW * currentScaleX;
    transforms.paddingY =
      (transforms.anchorY - tempBoundingH) * currentScaleY +
      tempBoundingH * currentScaleY;
  ...
  if (!transforms.isTranslate) {
    ...
    transforms.translate =
      "translate(" + transforms.paddingX + "," + transforms.paddingY + ") ";
  }
  ...
  while (i < objLength - 1) {
    ...
    if (offsetKeyframeObj[refKey].k[i].hasOwnProperty("_comp")) {
      ...
      if (
        ...
        if (offsetKeyframeObj[refKey].k[i + 1].i.x < 1)
          offsetKeyframeObj[refKey].k[i + 1].i.x = 0.0;
        if (offsetKeyframeObj[refKey].k[i + 1].i.y < 1)
          offsetKeyframeObj[refKey].k[i + 1].i.y = 0.0;
      }
      if (
        ...
        if (offsetKeyframeObj[refKey].k[i].o.x < 1)
          offsetKeyframeObj[refKey].k[i].o.x = 0.0;
        if (offsetKeyframeObj[refKey].k[i].o.y < 1)
          offsetKeyframeObj[refKey].k[i].o.y = 0.0;
      }
      ...
      if (animation[animationId]._currentLayer == 133) {
        console.log(
          "returning: " +
            returnedKeyframeObj.length +
            "," +
            offsetKeyframeObj[refKey].k.length
        );
      }
...
function prepShapeSh(
  ...
  if (shapeObj.ks.k.hasOwnProperty("v")) {
    ...
    if (shapeObj.ks.k[0].hasOwnProperty("s")) {
      ...
      shapeObj = extrapolateOffsetKeyframe(
        shapeObj,
        "ks",
        false,
        animationId,
      ...
      for (var kCount = 0; kCount < totalK; kCount++) {
        ...
        //transforms.refObjOther = animationId + "_" + depth + "_layerGroup" + animation[animationId]._currentLayerGroup;
        transforms = findExistingTransform(
          transforms,
          animationId,
          shapeObj.ks.k[kCount].t
        );
        var dataString =
          "M" +
        ...
        for (var i = 1; i < shapeObj.ks.k[kCount].s[0].v.length; i++) {
          dataString =
            dataString +
            " C" +
            (shapeObj.ks.k[kCount].s[0].v[i - 1][0] +
              shapeObj.ks.k[kCount].s[0].o[i - 1][0]) +
            "," +
            (shapeObj.ks.k[kCount].s[0].v[i - 1][1] +
              shapeObj.ks.k[kCount].s[0].o[i - 1][1]) +
            " " +
            (shapeObj.ks.k[kCount].s[0].v[i][0] +
              shapeObj.ks.k[kCount].s[0].i[i][0]) +
            "," +
            (shapeObj.ks.k[kCount].s[0].v[i][1] +
              shapeObj.ks.k[kCount].s[0].i[i][1]) +
            " " +
            shapeObj.ks.k[kCount].s[0].v[i][0] +
            "," +
            shapeObj.ks.k[kCount].s[0].v[i][1];
        ...
        if (shapeObj.ks.k[0].s[0].c) {
          dataString =
            dataString +
            " C" +
            (shapeObj.ks.k[kCount].s[0].v[
              shapeObj.ks.k[kCount].s[0].v.length - 1
            ][0] +
              shapeObj.ks.k[kCount].s[0].o[
                shapeObj.ks.k[kCount].s[0].v.length - 1
              ][0]) +
            "," +
            (shapeObj.ks.k[kCount].s[0].v[
            ...
            " " +
            (shapeObj.ks.k[kCount].s[0].v[0][0] +
              shapeObj.ks.k[kCount].s[0].i[0][0]) +
            "," +
            (shapeObj.ks.k[kCount].s[0].v[0][1] +
              shapeObj.ks.k[kCount].s[0].i[0][1]) +
            " " +
            shapeObj.ks.k[kCount].s[0].v[0][0] +
            "," +
            shapeObj.ks.k[kCount].s[0].v[0][1];
  ...
  if (shapeObj.ks.k.c) {
    dataString =
      dataString +
      " C" +
      (shapeObj.ks.k.v[shapeObj.ks.k.v.length - 1][0] +
        shapeObj.ks.k.o[shapeObj.ks.k.v.length - 1][0]) +
      "," +
      (shapeObj.ks.k.v[shapeObj.ks.k.v.length - 1][1] +
        shapeObj.ks.k.o[shapeObj.ks.k.v.length - 1][1]) +
      " " +
...
function createGradientDef(start, end, opacity, gradient, animationId) {
  ...
  if (gradient.k.k.length > gradient.p * 4) {
    for (var i = 0; i < gradient.p; i++) {
      opacities[i] =
        "stop-opacity:" + gradient.k.k[i * 2 + gradient.p * 4 + 1] + ";";
      /*var newStop = document.createElementNS(xmlns, 'stop');
  ...
  for (var i = 0; i < shapesGroup.length; i++) {
    if (shapesGroup[i]._isShape) {
      ...
      document
        ...
        .setAttribute("stroke", strokeToSet.color);
      document
        .getElementById(animationId + "_shape" + shapesGroup[i]._shape)
      ...
      if (layerObj.shapes[i].ty == "tr") {
        ...
        if (layerObj.shapes[i].p.hasOwnProperty("k")) {
          if (layerObj.shapes[i].p.k > 1) {
            document
              .getElementById(
                animationId + "_" + depth + "_layerGroup" + layerObj._layer
              )
              .setAttribute(
                "transform",
                "matrix(1,0,0,1," +
                  layerObj.shapes[i].p.k[0] +
                  "," +
                  layerObj.shapes[i].p.k[1] +
                  ")"
              );
          }
        }
      ...
      document
        .getElementById(
          animationId +
            ...
            depth +
            "_layerTranslate" +
            passedObj[passedKey][layerId]._parent
        )
        .prepend(newLayer);
...
function getLayers(
  ...
  for (var i = 0; i < passedObj.layers.length; i++) {
    ...
    if (
      passedObj[passedKey][i].hasOwnProperty("ip") &&
      passedObj[passedKey][i].ip >= 0
    ) {
    ...
    passedObj.layerCount = passedObj[passedKey][i]._layer;
    newLayer = document.getElementById(
      animationId + "_" + depth + "_layer" + passedObj[passedKey][i]._layer
    );
    newGroup = document.getElementById(
      ...
      if (passedObj[passedKey][i].ks.hasOwnProperty("a")) {
        if (passedObj[passedKey][i].ks.a.hasOwnProperty("k")) {
          if (passedObj[passedKey][i].ks.a.k.length > 1) {
            passedObj[passedKey][i]._anchorX =
          ...
          if (passedObj[passedKey][i].ks.p.k.length > 1) {
            if (passedObj[passedKey][i].ks.p.k[0].hasOwnProperty("s")) {
              ...
              if (passedObj[passedKey][i]._anchorX != 0) {
                posX =
                  passedObj[passedKey][i].ks.p.k[0] -
                  passedObj[passedKey][i]._anchorX;
              } else {
              ...
              if (passedObj[passedKey][i]._anchorY != 0) {
                posY =
                  passedObj[passedKey][i].ks.p.k[1] -
                  passedObj[passedKey][i]._anchorY;
              } else {
...
function buildGraph(
  ...
  newSVG.setAttributeNS(
    null,
    "viewBox",
    "0 0 " + animation[animationId].w + " " + animation[animationId].h
  );
